...
class OptTools:
    ...
    def set_features(self, D=[], X=[], Y=[]):
        ...
        self.YD_features_index = (
            self.dfJoint.groupby(self.Y_features + self.D_features)["Frequency"]
            .sum()
            .index
        )
    ...
    def set_distortion(
        ...
        msgtag = "opt_tools.py > set_distortion,optim_val확인".split(",")
        print("\n\n----------S : [{}]{}----------".format(msgtag[0], msgtag[1]))
        print(optim_val, self.dfD.shape[1], rows_dict)
        print("|||||||")
        print(self.dfD)
        print("│││││││││││││")
        # print(optim_val[0][0] + optim_val[0][2] + str(optim_val[0][1]))
        print("==========E : [{}]{}==========".format(msgtag[0], msgtag[1]), end="\n\n")

        print(rows_tuple)

        # for i in range(len(optim_val)):
        #     if optim_val[i][1]:
        #         optim_val[i].append(optim_val[0] + optim_val[2] + str(optim_val[1]))

        # rows_dict = [dict([a, float()])]

        if select_val in base_file:
            ...
            for i in range(self.dfD.shape[0]):
                ...
                for j in range(self.dfD.shape[1]):
                    new_values = cols_dict[j]
                    self.dfD.iat[i, j] = get_distortion(
                        old_values,
                        new_values,
                        select_protected,
                        select_features,
        ...
        msgtag = "opt_tools.py > set_distortion,끝난뒤...".split(",")
        print("\n\n----------S : [{}]{}----------".format(msgtag[0], msgtag[1]))
        print(self.dfD.iat)
        print("==========E : [{}]{}==========".format(msgtag[0], msgtag[1]), end="\n\n")
    ...
    def optimize(self, epsilon=1.0, dlist=[], verbose=True):
        ...
        PYhgD = Variable(
            (self.dfD_to_Y_address.shape[1], self.dfD_to_Y_address.shape[0])
        )
        ...
        Pxy_xhyh = (
            ...
            .dot(np.diag(PxydMarginal + 1e-10))
            * Pmap
        )
        ...
        msgtag = "opt_tools.py > optimize,prob 출력".split(",")
        print("\n\n----------S : [{}]{}----------".format(msgtag[0], msgtag[1]))
        print(prob)
        print("───────────────")
        print(prob.status)
        ...
        if prob.status in ["optimal", "optimal_inaccurate"]:
            print("Optimized Preprocessing: Objective converged to %f" % (prob.value))
        else:
            raise RuntimeError(
                "Optimized Preprocessing: Optimization did not " "converge"
            )
