...
class OptimPreproc(Transformer):
    ...
    def __init__(
        ...
        super(OptimPreproc, self).__init__(
            optimizer=optimizer,
        ...
        self.select_protect = select_protect
        ...
        if unprivileged_groups or privileged_groups:
            warn(
                "Privileged and unprivileged groups specified will not be "
    ...
    def fit(self, dataset, sep="="):
        ...
        if len(np.unique(dataset.instance_weights)) > 1:
            warn(
                "Optimized pre-processing will ignore instance_weights in "
                "the dataset during fit."
            )
        # Convert the dataset to a dataframe and preprocess
        df, _ = dataset.convert_to_dataframe(
            de_dummy_code=True, sep=sep, set_category=True
        )
        ...
        self.X_feature_names = [
            ...
            if n not in self.Y_feature_names and n not in self.protected_attribute_names
        ]
        self.feature_names = (
            self.X_feature_names + self.Y_feature_names + self.protected_attribute_names
        )

        msgtag = "optim_preproc.py > fit(),선택한 항목들 확인".split(",")
        print("\n\n----------S : [{}]{}----------".format(msgtag[0], msgtag[1]))
        print(self.protected_attribute_names)
        print(self.privileged_protected_attributes)
        print(self.unprivileged_protected_attributes)
        print(self.feature_names)
        print(self.X_feature_names)
        print(self.Y_feature_names)
        ...
        self.OpT.set_features(
            D=self.protected_attribute_names,
        ...
        self.OpT.set_distortion(
            ...
            self.optim_val,
            self.label,
            self.optim_options["distortion_fun"],
    ...
    def transform(self, dataset, sep="=", transform_Y=True):
        ...
        if (
            ...
            or D_feature_names != self.protected_attribute_names
        ):
        ...
        if transform_Y and (Y_feature_names != self.Y_feature_names):
            raise ValueError(
                "The label names must match with that in the training dataset"
            )
        ...
        df_dum = pd.concat(
            [
                pd.get_dummies(df_transformed.loc[:, X_feature_names], prefix_sep="="),
    ...
    def fit_transform(self, dataset, sep="=", transform_Y=True):
        ...
        return self.fit(dataset, sep=sep).transform(
            dataset, sep=sep, transform_Y=transform_Y
        )
...
def _apply_randomized_mapping(df, dfMap, features=[], random_seed=None):
    ...
    rem_cols = [l for l in df.columns if l not in features]
    if rem_cols != []:
