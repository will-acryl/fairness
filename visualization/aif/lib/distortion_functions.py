...
def get_distortion_german(vold, vnew):
    ...
    distort["employment"] = pd.DataFrame(
        {
            "Unemployed": [0.0, 1.0, 2.0],
            "1-4 years": [1.0, 0.0, 1.0],
            "4+ years": [2.0, 1.0, 0.0],
        },
    ...
    distort["status"] = pd.DataFrame(
        {"None": [0.0, 1.0, 2.0], "<200": [1.0, 0.0, 1.0], "200+": [2.0, 1.0, 0.0]},
        index=["None", "<200", "200+"],
    )
    distort["credit"] = pd.DataFrame(
        {"Bad Credit": [0.0, 1.0], "Good Credit": [2.0, 0.0]},
        index=["Bad Credit", "Good Credit"],
    )
    distort["sex"] = pd.DataFrame({0.0: [0.0, 2.0], 1.0: [2.0, 0.0]}, index=[0.0, 1.0])
    distort["age"] = pd.DataFrame({0.0: [0.0, 2.0], 1.0: [2.0, 0.0]}, index=[0.0, 1.0])
