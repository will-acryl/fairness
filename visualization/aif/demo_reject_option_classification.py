...
## import dataset
class rej_option_classification:
    def Run(
        select_val,
        select_protect,
        ...
        if dataset_used == "adult":
            ...
            msgtag = "demo_reject_option_classification > Run, reject에서 preproc들어가기 전".split(
                ","
            )
            ...
            if protected_attribute_used:
                ...
                msgtag = "demo_reject_option_classification > Run, reject에서 preproc해서 나온 dataset_orig 바로 출력".split(
                    ","
                )
                print("\n\n----------S : [{}]{}----------".format(msgtag[0], msgtag[1]))
                print(dataset_orig)
        ...
        # result.append(metric_valid_bef["Balanced accuracy"])
        # result.append(metric_valid_bef["Statistical parity difference"])
        # result.append(metric_valid_bef["Disparate impact"])
        # result.append(metric_valid_bef["Equal opportunity difference"])
        # result.append(metric_valid_bef["Average odds difference"])
        # result.append(metric_valid_bef["Theil index"])
        ...
        # result.append(metric_valid_aft["Balanced accuracy"])

        # In[13]:

        # Testing: Check if the metric optimized has not become worse
        assert np.abs(metric_valid_aft[metric_name]) <= np.abs(
            metric_valid_bef[metric_name]
        )
